...
func (h *handler) Unsubscribe(ctx context.Context, tgChatID, tgUserID int64, roleName string) error {
	chat, err := h.repo.FindChat(ctx, tgChatID)
	if err != nil {
		return err
	}
	...
	return h.repo.UnassignRole(ctx, chat.ID, role.ID, chatUser.ID)
}
